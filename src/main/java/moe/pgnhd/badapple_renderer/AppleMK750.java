...
import pl.socketbyte.cm4j.SDKDLLInterface;

public class AppleMK750 {
	...
	private static void printFrame(BufferedImage img, SDKDLLInterface kbi, int devIndex) {
		char dark = '#';
		char bright = ' ';
		...
		for(int y=0; y<yRes; y++) {
			for(int x=0; x<xRes; x++) {
				...
				cachedSetLedColor(x, y, new int[] {r,g,b}, kbi, devIndex);
			}
			frameBuffer.append("|\n");
		}
	...
	static int keyboardMatrix[][][] = new int[28][9][3];
	public static void cachedSetLedColor(int x, int y, int[] rgb, SDKDLLInterface kbi, int devIndex) {
		for(int i=0; i<3; i++) {
			if(keyboardMatrix[x][y][i] != rgb[i]) {
				keyboardMatrix[x][y][i] = rgb[i];
				kbi.SetLedColor(y, x, (byte) rgb[0], (byte) rgb[1], (byte) rgb[2], devIndex);
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// replace with correct keyboard
		int devIndex = SDKDLLInterface.DEVICE_INDEX.DEV_MK750.getValue(); 
		SDKDLLInterface kbi = SDKDLLInterface.INSTANCE;
		
		kbi.SetControlDevice(devIndex);
		kbi.EnableLedControl(true, devIndex);
		...
		for(;;) {
			...
			try {
				printFrame(mat2buffereImage(frame), kbi, devIndex);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
